...
def complaints():
    if request.method == 'POST':
        ...
        if session['username'] == 'vendor':
            ...
            complaints = conn.execute(text('select distinct c.complaint_id, c.status as complaint_status, c.date, u.first_name, u.last_name, c.title as complaint_title, c.user_id, c.order_id, c.description as complaint_description, c.demands, c.variant_id, pv.variant_id, o.order_id, o.order_date, oi.order_id, oi.quantity, pv.product_id, p.category, p.title, p.description, p.vendor_id from complaints c join product_variants pv on c.variant_id=pv.variant_id join orders o on o.order_id=c.order_id join order_items oi on oi.variant_id=pv.variant_id and oi.order_id=c.order_id join products p on p.product_id=pv.product_id join users u on u.user_id=o.user_id where p.vendor_id=:vendor_id').params(vendor_id=vendor_id))
            return render_template('complaints.html', complaints=complaints)
        elif session['username'] == 'admin':
            complaints = conn.execute(text('select distinct c.complaint_id, c.status as complaint_status, c.date, c.title as complaint_title, u.first_name, u.last_name, c.user_id, c.order_id, c.description as complaint_description, c.demands, c.variant_id, pv.variant_id, o.order_id, o.order_date, oi.order_id, oi.quantity, pv.product_id, p.category, p.title, p.description, p.vendor_id from complaints c join product_variants pv on c.variant_id=pv.variant_id join orders o on o.order_id=c.order_id join order_items oi on oi.variant_id=pv.variant_id and oi.order_id=c.order_id join products p on p.product_id=pv.product_id join users u on u.user_id=o.user_id order by c.complaint_id'))
            return render_template('complaints.html', complaints=complaints)
        else:
            user_id = session['user_id']
            complaints = conn.execute(text('select distinct c.complaint_id, c.status as complaint_status, c.date, c.title as complaint_title, c.user_id, u.first_name, u.last_name, c.order_id, c.description as complaint_description, c.demands, c.variant_id, pv.variant_id, o.order_id, o.order_date, oi.order_id, oi.quantity, pv.product_id, p.category, p.title, p.description, p.vendor_id from complaints c join product_variants pv on c.variant_id=pv.variant_id join orders o on o.order_id=c.order_id join order_items oi on oi.variant_id=pv.variant_id and oi.order_id=c.order_id join products p on p.product_id=pv.product_id join users u on u.user_id=o.user_id where u.user_id=:user_id').params(user_id=user_id))
            return render_template('complaints.html', complaints=complaints)
